...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!conversationId || !senderId || !text) return NextResponse.json({ error: "conversationId, senderId, and text required" }, { status: 400 });
    const db = await getDB();
    if (!db) throw new Error("DB failed");
    const message = {
    ...
    try { await pusherServer.trigger("conversation-" + conversationId, "new-message", { _id: result.insertedId.toString(), ...message }); } catch (e) {}
