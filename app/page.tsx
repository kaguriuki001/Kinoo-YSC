...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { const u = JSON.parse(saved); if (u?.roles) setIsLoggedIn(true); } catch (e) {}
    }

    fetch("/api/users?t=" + Date.now())
      .then(r => r.json())
      .then(d => { if (Array.isArray(d)) setStats(prev => ({ ...prev, members: d.filter((u: any) => u.status === 'active').length })); })
      .catch(() => {});
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a1a2e, #16213e, #0f3460)', color: 'white', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '20px 30px' }}>
        {isLoggedIn ? (
          <Link href="/dashboard" style={{ background: 'white', color: '#1a1a2e', padding: '10px 24px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px' }}>
            Go to Dashboard →
          </Link>
        ) : (
          <Link href="/login" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '10px 24px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', border: '1px solid rgba(255,255,255,0.3)' }}>
            Sign In →
          </Link>
        )}
      </div>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '0 20px 30px' }}>
        ...
        <div style={{ background: 'white', color: '#1a1a2e', padding: '30px', borderRadius: '16px', marginBottom: '30px', textAlign: 'center' }}>
          ...
          {qrUrl && <img src={qrUrl} alt="Registration QR Code" style={{ width: '220px', height: '220px', margin: '0 auto 20px', display: 'block', borderRadius: '12px' }} />}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '30px' }}>
          <div style={{ background: 'rgba(255,255,255,0.1)', padding: '25px', borderRadius: '16px' }}>
            ...
            <p style={{ fontSize: '14px', opacity: '0.85', lineHeight: '1.6' }}>A disciplined, self-sustaining youth enterprise empowering members and community through unbreakable peer accountability and financial independence.</p>
          </div>
          <div style={{ background: 'rgba(255,255,255,0.1)', padding: '25px', borderRadius: '16px' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '10px' }}>🚀 Our Mission</h3>
            <p style={{ fontSize: '14px', opacity: '0.85', lineHeight: '1.6' }}>To build from a handful to a disciplined youth movement through strategic registration, enterprise development, sports, and daily peer accountability.</p>
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', padding: '20px 0', opacity: '0.7', fontSize: '13px' }}>
          <p style={{ marginBottom: '10px' }}>
            {isLoggedIn ? (
              <Link href="/dashboard" style={{ color: 'white', textDecoration: 'underline' }}>Go to Dashboard</Link>
            ) : (
              <Link href="/login" style={{ color: 'white', textDecoration: 'underline' }}>Member Login</Link>
            )}
          </p>
