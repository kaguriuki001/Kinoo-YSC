...
export default function EventsPage() {
  ...
  const loadEvents = () => {
    setLoading(true);
    fetch("/api/events-view?sort=" + sort + "&userId=" + userId + "&t=" + Date.now(), { cache: "no-store" })
      .then(r => r.json())
      .then(d => { if (Array.isArray(d)) setEvents(d); setLoading(false); })
      .catch(() => setLoading(false));
  };

  const selfCheckIn = async (eventId: string, title: string) => {
    if (!userId) { toast.error("Please log in"); return; }
    setCheckingIn(eventId);
    const res = await fetch("/api/events/self-checkin", {
      method: "POST",
    ...
    setCheckingIn(null);
    if (res.ok) {
      toast.success(data.already ? "You already checked in" : "Checked in at " + title + "!");
      loadEvents();
  ...
  return (
    <div style={{ color: textColor }}>
      ...
      {events.length === 0 ? <div style={cardStyle}><p style={{ opacity: "0.7", textAlign: "center" }}>No events yet.</p></div> : events.map((e: any) => (
        <div key={e._id} style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
            <div style={{ flex: 1, minWidth: "200px" }}>
              ...
              <p style={{ fontSize: "12px", color: "#10b981", marginTop: "6px" }}>✅ {e.attendanceCount || 0} checked in</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "6px", alignItems: "flex-end" }}>
              <span style={{ background: e.status === "completed" ? "#64748b" : "#3b82f6", color: "white", padding: "4px 12px", borderRadius: "20px", fontSize: "11px", textTransform: "uppercase", fontWeight: "600" }}>{e.status}</span>
          ...
          <div style={{ marginTop: "15px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {e.canCheckIn && !e.alreadyCheckedIn ? (
              <button onClick={() => selfCheckIn(e._id, e.title)} disabled={checkingIn === e._id} style={{ background: "#10b981", color: "white", border: "none", padding: "10px 20px", borderRadius: "8px", cursor: "pointer", fontSize: "14px", fontWeight: "600", opacity: checkingIn === e._id ? 0.6 : 1 }}>
                {checkingIn === e._id ? "Checking in..." : "✅ I'm Here — Check In"}
              </button>
            ) : e.alreadyCheckedIn ? (
              <span style={{ background: "#d1fae5", color: "#065f46", padding: "10px 20px", borderRadius: "8px", fontSize: "14px", fontWeight: "600" }}>✅ You checked in</span>
            ) : null}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 999, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "20px", overflowY: "auto" }} onClick={() => setSelectedEvent(null)}>
          <div style={{ background: darkMode ? "#1e293b" : "white", padding: "25px", borderRadius: "16px", maxWidth: "700px", width: "100%", color: textColor, marginTop: "20px" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: "20px", padding: "15px", background: darkMode ? "#334155" : "#f8fafc", borderRadius: "10px", fontSize: "14px", lineHeight: "1.7" }}>
              ...
              <p><strong>✅ Checked in:</strong> {selectedEvent.attendanceCount} members</p>
            </div>
            <div>
              <h3 style={{ fontSize: "18px", fontWeight: "bold", marginBottom: "12px" }}>💬 Member Feedback ({feedback.length})</h3>
              {feedback.length === 0 ? <p style={{ opacity: "0.7", fontSize: "14px" }}>No feedback yet.</p> : (
                <div>
                  ...
                  {feedback.map((fb: any) => (
                    <div key={fb._id} style={{ padding: "12px", borderRadius: "8px", marginBottom: "10px", background: darkMode ? "#334155" : "#f8fafc", border: "1px solid #e5e7eb" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                        ...
                        {canCurate && <button onClick={() => toggleFeedback(fb._id, selectedEvent._id, fb.includeInFrago !== false)} style={{ background: "#3b82f6", color: "white", border: "none", padding: "4px 10px", borderRadius: "5px", cursor: "pointer", fontSize: "11px" }}>Toggle</button>}
                      </div>
