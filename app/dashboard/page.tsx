...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch("/api/users", { credentials: "include" }).then(r => r.json()).then(d => {
      if (Array.isArray(d)) { const active = d.filter((u: any) => u.status === 'active'); setMembers(active); setFilteredMembers(active); setStats(p => ({ ...p, members: active.length })); }
    });
    fetch("/api/events", { credentials: "include" }).then(r => r.json()).then(d => { if (Array.isArray(d)) { setEvents(d); setStats(p => ({ ...p, events: d.length })); } });
    fetch("/api/transactions", { credentials: "include" }).then(r => r.json()).then(d => {
      if (Array.isArray(d)) { const total = d.filter((t: any) => t.verified).reduce((s: number, t: any) => s + t.amount, 0); setStats(p => ({ ...p, transactions: d.length, balance: total })); }
    });
  }, []);
  ...
  if (!user) return <div style={{ padding: '40px', textAlign: 'center', fontSize: '18px', color: darkMode ? '#e2e8f0' : '#333' }}>Loading...</div>;

  const textColor = darkMode ? '#e2e8f0' : '#1e293b';
  const subTextColor = darkMode ? '#94a3b8' : '#666';
  ...
  const tabStyle = (active: boolean) => ({ padding: '10px 20px', border: 'none', borderRadius: '8px', cursor: 'pointer', background: active ? '#3b82f6' : darkMode ? '#334155' : '#e5e7eb', color: active ? 'white' : textColor, fontSize: '14px' });
  ...
  return (
    <div style={{ color: textColor }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '15px', marginBottom: '20px' }}>
        {[{ label: 'Members', value: stats.members, color: '#3b82f6', icon: '👥' }, { label: 'Events', value: stats.events, color: '#f59e0b', icon: '📅' }, { label: 'Transactions', value: stats.transactions, color: '#10b981', icon: '💰' }, { label: 'Balance', value: `KES ${stats.balance.toLocaleString()}`, color: '#8b5cf6', icon: '💎' }].map(c => (
          <div key={c.label} style={{ ...cardStyle, textAlign: 'center' }}>
            ...
            <p style={{ color: subTextColor, fontSize: '13px' }}>{c.label}</p>
          </div>
        ))}
      </div>
      ...
      {activeTab === 'overview' && (
        <div style={cardStyle}>
          <h3 style={{ marginBottom: '15px', color: textColor }}>Quick Actions</h3>
      ...
      {activeTab === 'events' && (
        <div style={cardStyle}>
          <h3 style={{ marginBottom: '15px', color: textColor }}>Upcoming Events ({events.length})</h3>
          {events.length === 0 ? <p style={{ color: subTextColor }}>No events.</p> : events.map((e: any) => (
            <div key={e._id} style={{ padding: '15px', border: `1px solid ${darkMode ? '#334155' : '#f0f0f0'}`, borderRadius: '8px', marginBottom: '10px' }}>
              <h4 style={{ marginBottom: '5px', color: textColor }}>{e.title}</h4>
              <p style={{ fontSize: '14px', color: subTextColor }}>{new Date(e.date).toDateString()} - {e.venue}</p>
              <p style={{ fontSize: '14px', color: subTextColor }}>Ticket: KES {e.ticketPrice || 'Free'}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {activeTab === 'attendance' && (
        <div style={cardStyle}>
          <h3 style={{ marginBottom: '15px', color: textColor }}>Attendance Tracking</h3>
      ...
      {activeTab === 'giving' && (
        <div style={cardStyle}>
          <h3 style={{ marginBottom: '15px', color: textColor }}>Contribute</h3>
      ...
      {activeTab === 'members' && (
        <div style={cardStyle}>
          <h3 style={{ marginBottom: '15px', color: textColor }}>Members ({filteredMembers.length})</h3>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              <th style={{ textAlign: 'left', padding: '12px', borderBottom: `2px solid ${darkMode ? '#334155' : '#e5e7eb'}`, fontSize: '13px', color: subTextColor }}>Name</th>
              <th style={{ textAlign: 'left', padding: '12px', borderBottom: `2px solid ${darkMode ? '#334155' : '#e5e7eb'}`, fontSize: '13px', color: subTextColor }}>Phone</th>
              <th style={{ textAlign: 'left', padding: '12px', borderBottom: `2px solid ${darkMode ? '#334155' : '#e5e7eb'}`, fontSize: '13px', color: subTextColor }}>Status</th>
            </tr></thead>
            <tbody>
              {filteredMembers.map((m: any) => (
                <tr key={m._id}>
                  <td style={{ padding: '12px', borderBottom: `1px solid ${darkMode ? '#334155' : '#f0f0f0'}`, fontSize: '14px', color: textColor }}>{m.fullName}</td>
                  <td style={{ padding: '12px', borderBottom: `1px solid ${darkMode ? '#334155' : '#f0f0f0'}`, fontSize: '14px', color: textColor }}>{m.phone}</td>
