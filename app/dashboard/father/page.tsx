...
export default function FatherPage() {
  ...
  const fetchAuditLogs = async () => {
    try { const res = await fetch("/api/audit"); const data = await res.json(); if (Array.isArray(data)) setAuditLogs(data); } catch (e) {}
  };
  const logAction = async (action: string, targetUser?: string, reason?: string) => {
    await fetch("/api/audit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, performedBy: "father", targetUser, reason }) });
    fetchAuditLogs();
  };
  const dismissMember = async (id: string, name: string) => {
    if (!confirm(`Dismiss ${name}?`)) return;
    const res = await fetch("/api/bulk", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: 'delete', userIds: [id] }) });
    if (res.ok) { toast.success(`${name} dismissed`); await logAction("dismiss_member", id, `Dismissed ${name}`); fetchMembers(); }
  };
  const freezeTreasury = async () => {
    if (!confirm("Freeze all transactions?")) return;
    await logAction("freeze_treasury", undefined, "Treasury frozen");
    toast.success("Treasury frozen");
  };
  const resetPassword = async (id: string, name: string) => {
    ...
    toast.success(`${name}'s password: Kinoo123!`);
  ...
  return (
    <div style={{ color: textColor }}>
      ...
      <div style={{ display: 'flex', gap: '5px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {tabs.map(t => (<button key={t.id} onClick={() => setActiveTab(t.id)} style={{ padding: '10px 20px', border: 'none', borderRadius: '8px', cursor: 'pointer', background: activeTab === t.id ? '#dc2626' : darkMode ? '#334155' : '#e5e7eb', color: activeTab === t.id ? 'white' : textColor, fontSize: '14px' }}>{t.label}</button>))}
      </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
            <div style={cardStyle}>
              ...
              <button onClick={freezeTreasury} style={{ background: '#dc2626', color: 'white', border: 'none', padding: '12px', borderRadius: '8px', cursor: 'pointer', width: '100%', fontWeight: '600' }}>❄️ Freeze Treasury</button>
            </div>
            <div style={cardStyle}>
